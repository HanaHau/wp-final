...
export default function DecorPanel({
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 bottom-20 z-[60] bg-white border-l-4 border-black w-full max-w-sm flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b-2 border-black bg-white">
          <h2 className="text-xl font-bold uppercase tracking-wide">裝飾</h2>
          <button
            onClick={onClose}
            ...
            {availableAccessories.length === 0 ? (
              ...
              availableAccessories.map((accessory) => (
                ...
                >
                  ...
                  {/* Text labels */}
                  <div className="text-[10px] font-bold uppercase text-center line-clamp-1 mb-1 pointer-events-none select-none">
                    {accessory.name}
                  </div>
                  <div className="text-xs font-bold pointer-events-none select-none">×{accessory.count}</div>
                </div>
              ))
            )}
          </div>
